...
export class RabbitMQPublisher {
  ...
  async publish(message: unknown, options?: PublishOptions): Promise<boolean> {
  ...
  async publishToExchange(
    ...
    const logContext = {
      ...
      routingKey,
      messageType:
        typeof message === "object" ? (message as any)?.type : undefined,
    };
    ...
    try {
      await this.connection.withChannel(async (channel) => {
        try {
          ...
          lg.trace(
            { ...logContext, messageSize: messageString.length },
            "Serialized message for publishing"
          );
          ...
          lg.debug(
            { ...logContext },
            "Message successfully published to exchange"
          );
        } catch (publishError) {
          lg.error(
            { ...logContext, error: publishError },
      ...
      lg.error(
        { ...logContext, error: channelError },
        "Channel operation failed during publish"
      );
      throw channelError;
    }
  }
  ...
  async close(): Promise<void> {
    ...
    try {
      ...
      lg.error(
        { ...logContext, error },
        "Failed to close publisher connection"
      );
      throw error;
    }
  }
