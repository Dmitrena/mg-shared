...
export class RabbitMQConsumer {
    ...
    async consume(handler, options) {
        ...
        return this.connection.withChannel(async (channel) => {
            ...
            const { consumerTag } = await channel.consume(this.config.queue, async (msg) => {
                if (!msg) {
    ...
    async cancel(consumerTag) {
        ...
        if (!targetConsumerTag) {
            lg.warn(logContext, "Attempted to cancel consumer with no active consumerTag");
            return;
        }
        ...
        try {
            await this.connection.withChannel(async (channel) => {
                try {
                    ...
                    lg.error({ ...logContext, error }, "Failed to cancel consumer on channel");
                    throw error;
                }
            });
        }
        catch (error) {
            lg.error({ ...logContext, error }, "Channel operation failed during consumer cancellation");
            throw error;
        }
    }
    async close() {
        ...
        try {
            if (this.consumerTag) {
                lg.debug({ ...logContext, consumerTag: this.consumerTag }, "Active consumer found, cancelling first");
                await this.cancel();
            }
